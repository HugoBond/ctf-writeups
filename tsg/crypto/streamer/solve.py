...
flag_length = 304
first_bytes= b"TSGCTF{"
known_bytes =[(278,"@"),(301,"="), (302,"="), ((303,"}"))]

keystream = [first_bytes[i] ^ cipher[i] for i in range(7)]
keystream += [cipher[71] ^ ord('e'), cipher[216] ^ ord('h'), cipher[265] ^ ord('a'), cipher[266] ^ ord('h'), cipher[267] ^ ord('a'), 48 ^ cipher[12]]
keystream += [cipher[301] ^ ord('=') ,cipher[302] ^ ord('='), cipher[303] ^ ord('}')]

def decrypt(keystream):
    flag = ""
    for i in range(flag_length):
        flag += chr(cipher[i] ^ keystream[i % 16])
    return flag

flag = decrypt(keystream)
print(flag)
